...
const sendWhatsAppMessage = async ({ phoneNo, message, numberKey, sessionId }) => {
    const provider = String(process.env.WA_PROVIDER || "watzap").toLowerCase();

    if (["baileys", "redis", "socket"].includes(provider)) {
        const { sendWhatsAppSocketMessage } = require("./baileysWaService");

        return sendWhatsAppSocketMessage({
            sessionId: sessionId || process.env.WA_SESSION_ID || "main",
            phoneNo,
            message,
        });
    }
...
const sendWhatsAppToMany = async ({ phoneNumbers = [], message, numberKey, sessionId }) => {
    const uniqueNumbers = [
